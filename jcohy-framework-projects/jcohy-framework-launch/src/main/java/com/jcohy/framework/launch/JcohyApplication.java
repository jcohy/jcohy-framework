 ...
 * @since 2022.0.1
 */
public final class JcohyApplication {
...
//				.main(source)
				...
				.listeners(new ApplicationEnvironmentPreparedListener())
				.properties(addProperties(applicationName));
		// 获取环境变量
		String profile = configurationProfile(args);
		builder.profiles(profile);

		String startJarPath = JcohyApplication.class.getResource("/").getPath().split("!")[0];
		System.out.printf("----启动中，读取到的环境变量:[%s]，jar地址:[%s]----%n", profile, startJarPath);


		List<LauncherService> services = new ArrayList<>();
		ServiceLoader.load(LauncherService.class).forEach(services::add);
		services.stream().sorted(Comparator.comparing(LauncherService::getOrder)).collect(Collectors.toList())
				.forEach((launcherService) -> launcherService.launcher(builder, applicationName, profile, isLocalDev(profile)));
